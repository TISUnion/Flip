...
def onServerInfo(server, info):
  if info.isPlayer == 1:
    if info.content.startswith('!!flip'):
      ...
      if len(args) == 1:
        server.tell(info.player, random.randint(0,1))
        ...
        server.tell(info.player, random.randint(0,int(float(args[1]))))
      elif (len(args) == 3) and (args[1].isdigit()) and (args[2].isdigit()) and (int(float(args[1])) > int(float(args[2]))):
        server.tell(info.player, random.randint(int(float(args[2])),int(float(args[1]))))
      else:
